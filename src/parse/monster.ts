...
export function parseMonster() {
    const read = fs.readFileSync(
        'scripts/Grasscutter_Resources/Resources/ExcelBinOutput/MonsterExcelConfigData.json',
        'utf-8'
    )
    ...
    Object.entries(
        ...
        v.forEach(
            ({ id, monsterName }) =>
                (monsterObj[k as MonsterType][id] = monsterName)
        )
    )
    ...
    // console.log(
    //     monsterJson
    //         .filter(item => diff_id_monster.includes(item.id))
    //         .map(({ id, monsterName, type }) => ({ id, monsterName, type }))
    // )
}
